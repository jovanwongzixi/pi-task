/**
 * Tmux helpers and pane backend for subagent panes.
 */
...
export function tmuxCmd(args: string[], input?: string): string {
...
function shouldKillPane(outcome: SubagentOutcome): boolean {
...
export function setPaneSelfDestruct(
  paneId: string,
  enabled: boolean,
  delaySeconds = 1,
): void {
...
export function paneExists(paneId: string): boolean {
  return (
    tmuxCmdQuiet(["display-message", "-p", "-t", paneId, "#{pane_id}"]) ===
    paneId
  );
}

export function paneDead(paneId: string): boolean {
  const value = tmuxCmdQuiet([
    "display-message",
    "-p",
    "-t",
    paneId,
...
export function tmuxSteerPane(paneId: string, message: string): void {
  ...
  try {
    tmuxCmd(["load-buffer", "-b", bufferName, "-"], message);
...
function sessionWatcherScript(sessionFilePath: string): string {
  ...
  // Watch a specific file so stale sibling files cannot trigger premature exit.
  return `(
...
export class TmuxBackend implements SubagentPaneBackend {
  ...
  spawn(options: SpawnSubagentOptions): SpawnedSubagentPane {
    ...
    const command = options.sessionFilePath
      ? wrapWithPaneExitWatcher(options.sessionFilePath, options.command)
      : options.command;
  ...
  finalize(
    ...
    if (!paneId || !shouldKillPane(outcome)) return;
    try {
      this.command(["kill-pane", "-t", paneId]);
    } catch {
      // Pane already exited or self-cleaned.
    }
  }
